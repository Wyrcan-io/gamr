...
export function renderFrame(state: GameState, cols: number, rows: number, theme: string, glitchFrame: number): string {
  ...
  if (state.phase === 'ending' || state.phase === 'gameOver') { const won = state.phase === 'ending'; const next = won && state.scenarioIndex === 0 ? 'N: NEXT SCENARIO' : 'N: NEXT GAME'; center(out, cols, 8, won ? '✓ EXTRACTION COMPLETE' : '⚠ LINE LOST', won ? GREEN + '\x1b[1m' : RED + '\x1b[1m'); center(out, cols, 11, `${state.evacuatedPeople} PEOPLE SAFE   ${state.evacuatedSupplies} SUPPLIES DELIVERED`, theme); state.eventLog.slice(0, 3).forEach((entry, i) => center(out, cols, 14 + i, entry.text, eventColor(entry.tone) || theme)); center(out, cols, 22, `R: RESTART   Q: QUIT   ${next}`, DIM + theme); return out.join(''); }
  ...
  if (selectedTrain) { put(out, panel, MAP_Y + 12, `${CYAN}\x1b[1mTRAIN ${selectedTrain.id}${RESET}`); put(out, panel, MAP_Y + 13, `${selectedTrain.name.slice(0, 18)}`); put(out, panel, MAP_Y + 14, `PEOPLE ${selectedTrain.people} LOAD ${selectedTrain.supplies}`); put(out, panel, MAP_Y + 15, `STATUS ${selectedTrain.status.toUpperCase().slice(0, 12)}`); }
  ...
  else put(out, 3, 25, `${DIM}${theme}MOVE  TAB TRAINS  1-4 ACTIONS  SPACE COMMIT  H HELP  ESC PAUSE${RESET}`);
