...
import { runStackTraceGame } from './index';
...
describe('Stack Trace controller lifecycle', () => {
  afterEach(() => vi.useRealTimers());

  it('enters the alternate buffer and restores it exactly once', () => {
    vi.useFakeTimers();
    const terminal = fakeTerminal();
    const controller = runStackTraceGame(terminal as unknown as Terminal);

    vi.advanceTimersByTime(60);
    expect(terminal.writes.join('')).toContain('\x1b[?1049h');
    expect(terminal.handlers).toHaveLength(1);
  ...
  it('opens contextual help without mutating the repair state', () => {
    vi.useFakeTimers();
    const terminal = fakeTerminal();
    const controller = runStackTraceGame(terminal as unknown as Terminal);
    vi.advanceTimersByTime(60);
    ...
    expect(terminal.writes.join('')).toContain('g/ STACK TRACE / HELP');
    controller.stop();
  });
});
