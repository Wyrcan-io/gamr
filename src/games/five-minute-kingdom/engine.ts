...
export type Command = { type: 'dismissBriefing' } | { type: 'selectOffer'; index: number } | { type: 'moveTarget'; dx: number; dy: number } | { type: 'preview' } | { type: 'cancelPreview' } | { type: 'confirm' } | { type: 'dismissResult' } | { type: 'dismissSeason' } | { type: 'restart'; seed?: number };
export function applyCommand(state: GameState, command: Command): GameState {
  ...
  switch (command.type) {
    ...
    case 'cancelPreview': if (next.phase === 'preview') { next.preview = null; next.phase = 'chooseTarget'; next.notice = 'PREVIEW CANCELLED. CHOOSE A SQUARE.'; } break;
