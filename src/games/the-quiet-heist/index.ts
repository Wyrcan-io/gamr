...
export function runTheQuietHeistGame(terminal: Terminal): TheQuietHeistController {
  ...
  function onKey(event: KeyboardEvent): void {
    ...
    if (key === '?') { help(); render(); return; }
    if (key === 'arrowleft' || key === 'left' || key === 'a') command({ type: 'move', direction: 'W' }); else if (key === 'arrowright' || key === 'right' || key === 'd') command({ type: 'move', direction: 'E' }); else if (key === 'arrowup' || key === 'up' || key === 'w') command({ type: 'move', direction: 'N' }); else if (key === 'arrowdown' || key === 'down' || key === 's') command({ type: 'move', direction: 'S' }); else if (key === 'i') command({ type: 'interact' }); else if (key === 'j') command({ type: 'jam' }); else if (key === 'x') command({ type: 'decoy' }); else if (key === 'u' || key === 'backspace') command({ type: 'undo' }); else if (key === 'enter' || key === ' ') command({ type: 'commit' }); else if (key === 'q') quit();
  }
