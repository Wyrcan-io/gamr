...
export function runFiveMinuteKingdomGame(terminal: Terminal): FiveMinuteKingdomController {
  ...
  function renderMarket(lines: string[], theme: string): void { box(lines, 8, 6, 82, 16, 'DRAFT MARKET', theme); at(lines, 12, 8, 'Choose one offer. You will then place it on the kingdom.'); state.market.forEach((offer, i) => { at(lines, 14, 11 + i * 3, `${i + 1}. ${labelFor(offer)}`); at(lines, 20, 12 + i * 3, `${DIM}${theme}${offer.kind === 'terrain' ? 'Builds a district and scores from matching neighbours.' : offer.kind === 'citizen' ? 'Homes need compatible terrain and score from their surroundings.' : 'A persistent law scores now and during season checks.'}${RESET}`); }); at(lines, 12, 22, `${DIM}${theme}1-3: DRAFT   ESC: PAUSE   Q: QUIT${RESET}`); }
  ...
  function renderSide(lines: string[], theme: string): void { box(lines, 46, 5, 51, 18, state.phase === 'season' ? 'SEASON REPORT' : state.phase === 'result' ? 'PLACEMENT RESULT' : 'CURRENT DECISION', theme); if (state.phase === 'chooseTarget' || state.phase === 'preview') { const offer = state.selectedOffer; at(lines, 50, 8, offer ? labelFor(offer) : 'No offer'); at(lines, 50, 10, `TARGET ${String.fromCharCode(65 + state.target.x)}${state.target.y + 1}: ${cellName(state.board[state.target.y]![state.target.x]!)}`); const legal = legalTarget(state, offer, state.target); at(lines, 50, 12, legal.legal ? 'LEGAL TARGET' : `! ${legal.reason}`); if (state.preview) state.preview.preview.forEach((text, i) => at(lines, 50, 14 + i, text)); at(lines, 50, 20, state.phase === 'preview' ? 'ENTER: CONFIRM' : 'ARROWS/WASD: MOVE  ENTER: PREVIEW'); } else if (state.phase === 'result') { state.lastEvents.slice(0, 5).forEach((event, i) => at(lines, 50, 8 + i * 2, `+${event.amount}  ${event.label}`)); at(lines, 50, 20, 'ENTER: CONTINUE'); } else if (state.phase === 'season') { state.seasonEvents.slice(0, 5).forEach((event, i) => at(lines, 50, 8 + i * 2, `+${event.amount}  ${event.label}`)); at(lines, 50, 20, 'ENTER: NEXT SEASON'); } at(lines, 3, 25, `${DIM}${theme}I: INSPECT   L: LEDGER   ESC: PAUSE   Q: QUIT${RESET}`); }
