...
export interface BellResolution { priceLines: string[]; factionLines: string[]; rumorLines: string[]; priceChanges: Record<GoodId, number>; }
...
function resolveBell(state: GameState): BellResolution {
  ...
  for (const artifact of state.artifacts) artifact.heldBells++; return { priceLines: priceLines.length ? priceLines : ['Prices held steady.'], factionLines: factionLines.length ? factionLines : ['The factions declined to move.'], rumorLines: rumorLines.length ? rumorLines : ['No rumor settled this bell.'], priceChanges };
}
...
export function applyCommand(input: GameState, command: { type: 'dismissBriefing' | 'previewAction' | 'confirmAction' | 'cancelPreview' | 'endDay' | 'dismissBellReport' | 'chooseMethod' | 'restart'; action?: Action; methodId?: MethodId; sameSeed?: boolean }): GameState {
  ...
  if (command.type === 'confirmAction' && state.phase === 'preview' && state.pending?.valid) { commitAction(state, state.pending); state.pending = null; state.phase = 'market'; if (state.actions <= 0) state.notice = 'Three actions spent. End the day when ready.'; return state; }
