...
import { deriveObservation, hullPips, orderLabel, previewSelectedOrder } from './engine';
...
function orderRows(state: GameState, width: number, palette: TerminalThemePalette): string[] {
  ...
  rows.push(row(`SELECTED ${preview.label}  ${preview.certainty}`, width, preview.legal ? palette.good : palette.warning));
  rows.push(row(preview.reason, width, palette.muted));
...
function replayRows(state: GameState, width: number, palette: TerminalThemePalette): string[] {
  const rows = [row(`PUBLIC REPLAY  STEP ${Math.min(state.replayIndex + 1, Math.max(1, state.reports.length))}/${Math.max(1, state.reports.length)}`, width, `${palette.focus}${BOLD}`)];
  state.reports.slice(0, state.replayIndex + 1).forEach(event => rows.push(row(`${event.kind === 'warning' ? '[!]' : event.kind === 'success' ? '[+]' : '[.]'} ${event.text}`, width, event.kind === 'warning' ? palette.warning : event.kind === 'success' ? palette.good : palette.ink)));
  rows.push(row('ENTER/RIGHT step  ESC/BACKSPACE finish replay', width, palette.muted));
  return rows;
}

function helpFrame(cols: number, palette: TerminalThemePalette): string {
  return ['\x1b[2J\x1b[H', row('g/ TINY FLEET / PLOTTING CARD', cols, `${palette.focus}${BOLD}`), '', row('Exact contacts are confirmed positions. Estimates are possible cells.', cols - 4, palette.ink), row('LEGAL means the order can be sealed; it does not guarantee a hit.', cols - 4, palette.ink), row('1-3 select ship  arrows aim  WASD helm  F fire  X special', cols - 4, palette.muted), row('ENTER review, ENTER again seal  TAB panel  ESC close', cols - 4, palette.muted)].join('\r\n');
}
