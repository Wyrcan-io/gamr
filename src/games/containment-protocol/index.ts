...
export function runContainmentProtocolGame(terminal: Terminal): ContainmentProtocolController {
  let running = true; let paused = false; let pauseSelection = 0; let state = createState(Date.now()); let listener: { dispose: () => void } | undefined; let resizeListener: { dispose: () => void } | undefined;
  ...
  const restart = () => { state = createState(state.seed, state.mode); paused = false; pauseSelection = 0; render(); };
  const command = (value: Command) => { state = applyCommand(state, value); render(); };
  ...
  function handlePause(key: string, event: KeyboardEvent): boolean { if (!paused) return false; const result = navigateMenu(pauseSelection, PAUSE_MENU_ITEMS.length, key, event); pauseSelection = result.newSelection; if (!result.confirmed) { if (key === 'escape') paused = false; render(); return true; } if (pauseSelection === 0) paused = false; else if (pauseSelection === 1) restart(); else if (pauseSelection === 2) quit(); else if (pauseSelection === 3) { controller.stop(); dispatchGamesMenu(terminal); } else if (pauseSelection === 4) { controller.stop(); dispatchGameSwitch(terminal); } render(); return true; }
  function onKey(event: KeyboardEvent): void {
    const key = event.key.toLowerCase(); event.preventDefault(); event.stopPropagation(); if (key === 'escape' && state.phase !== 'start' && state.phase !== 'ending') { paused = !paused; pauseSelection = 0; render(); return; } if (handlePause(key, event)) return;
  ...
  function render(): void { let output = renderFrame(state, terminal.cols, terminal.rows, getCurrentThemeColor(), 0); if (paused && terminal.cols >= 80 && terminal.rows >= 28) output += linePause(terminal.cols, terminal.rows, pauseSelection); terminal.write(output); }
  ...
  const originalStop = controller.stop; controller.stop = () => { if (!running) return; running = false; listener?.dispose(); resizeListener?.dispose(); terminal.write('\x1b[?25h\x1b[?1049l\x1b[0m'); originalStop(); };
  setTimeout(() => { if (!running) return; terminal.write('\x1b[?1049h\x1b[?25l'); listener = terminal.onKey(({ domEvent }) => { if (running) onKey(domEvent); }); resizeListener = terminal.onResize(() => render()); render(); }, 50);
  return controller;
}
