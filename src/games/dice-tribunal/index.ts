...
export function runDiceTribunalGame(terminal: Terminal): DiceTribunalController {
  ...
  function autoAssign(): void {
    ...
    for (const rolled of active.hearing.rolled) { if (assigned.has(rolled.dieId)) continue; const die = state.dice.find(item => item.id === rolled.dieId); const face = die?.faces[rolled.faceIndex]; if (!face || face.symbol === 'gaffe') continue; let target: Assignment['target'] | undefined;
      ...
      if (!target && face.symbol === 'objection' && !occupied.has(JSON.stringify({ kind: 'object' }))) target = { kind: 'object' }; if (!target && !occupied.has(JSON.stringify({ kind: 'clarify' }))) target = { kind: 'clarify' }; if (target) { command({ type: 'assignDie', assignment: { dieId: rolled.dieId, target } }); assigned.add(rolled.dieId); occupied.add(JSON.stringify(target)); }
    }
  }
  function handleKey(domEvent: KeyboardEvent): void {
    ...
    if (state.phase === 'hearing') { if (/^[1-5]$/.test(key)) { selection = Number(key) - 1; } else if (key === 'arrowright' || key === 'd') moveSelection(1, Math.max(1, state.dice.length)); else if (key === 'arrowleft' || key === 'a') moveSelection(-1, Math.max(1, state.dice.length)); else if (key === 'r' && domEvent.shiftKey) command({ type: 'rerollMarked' }); else if (key === 'r') { const id = selectedDieId(); if (id && state.activeCase?.hearing.rolled.length) command({ type: 'toggleRerollMark', dieId: id }); } else if (key === 'f') { const id = selectedDieId(); const evidence = state.activeCase?.selectedEvidenceIds.find(item => !state.activeCase?.admittedEvidenceIds.includes(item)); const slotIndex = evidence ? evidenceById(evidence)?.slots.findIndex(slot => slot.symbol === faceSymbol(state, id) && slot.minRank <= faceRank(state, id)) : -1; if (id && evidence && slotIndex !== undefined && slotIndex >= 0) command({ type: 'assignDie', assignment: { dieId: id, target: { kind: 'evidence', evidenceId: evidence, slotIndex } } }); } else if (key === 'o' || key === 'c') { const id = selectedDieId(); if (id) command({ type: 'assignDie', assignment: { dieId: id, target: { kind: key === 'o' ? 'object' : 'clarify' } } }); } else if (key === 'u') { const id = selectedDieId(); if (id) command({ type: 'unassignDie', dieId: id }); } else if (key === 'x') autoAssign(); else if (key === 'backspace') command({ type: 'cancelPreview' }); else if (key === 'enter') command({ type: !state.activeCase?.hearing.rolled.length ? 'roll' : state.pendingPreview ? 'confirmHearing' : 'previewHearing' }); return; }
  ...
  function render(): void { let output = renderFrame(state, terminal.cols, terminal.rows, getCurrentThemePalette(), { selection, helpOpen: help, paused }); if (paused && terminal.cols >= 80 && terminal.rows >= 28) output += renderSimpleMenu(PAUSE_MENU_ITEMS, pauseSelection, { centerX: Math.floor(terminal.cols / 2), startY: Math.floor(terminal.rows / 2) - 3, showShortcuts: false }); terminal.write(output); }
...
function faceSymbol(state: GameState, id: string | undefined): string | undefined { const rolled = state.activeCase?.hearing.rolled.find(item => item.dieId === id); const die = state.dice.find(item => item.id === id); return rolled && die ? die.faces[rolled.faceIndex]?.symbol : undefined; }
function faceRank(state: GameState, id: string | undefined): number { const rolled = state.activeCase?.hearing.rolled.find(item => item.dieId === id); const die = state.dice.find(item => item.id === id); return rolled && die ? die.faces[rolled.faceIndex]?.rank ?? 0 : 0; }
