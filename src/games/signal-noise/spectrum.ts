import type { CaseState, LockQuality, StationId, Transmitter } from './types';

export interface SpectrumReadout { energy: number[]; markers: string[]; }
export interface LockEvaluation { quality: LockQuality; reason: string; target: Transmitter; signal: number; noise: number; purity: number; }
...
export function evaluateLock(state: CaseState, station: StationId): LockEvaluation {
  ...
  if (state.disabledStations.includes(station)) return { quality: 'none', reason: 'STATION OFFLINE', target, signal, noise, purity };
  if (state.tuner.centre !== target.centre) return { quality: 'none', reason: 'CENTRE DOES NOT MATCH CARRIER', target, signal, noise, purity };
  if (state.tuner.bandwidth !== target.bandwidth) return { quality: 'none', reason: 'BANDWIDTH CLIPS OR ADMITS NOISE', target, signal, noise, purity };
  if (state.tuner.modulation !== target.modulation) return { quality: 'none', reason: 'MODULATION DOES NOT RESOLVE', target, signal, noise, purity };
  if (state.tuner.gain < target.requiredGainByStation[station]) return { quality: 'none', reason: 'GAIN TOO LOW', target, signal, noise, purity };
  if (state.tuner.gain > target.overloadGainByStation[station]) return { quality: 'none', reason: 'GAIN OVERLOAD', target, signal, noise, purity };
  ...
  return { quality, reason: quality === 'none' ? 'NOISE HIDES THE CARRIER' : quality.toUpperCase() + ' LOCK', target, signal, noise, purity };
}
