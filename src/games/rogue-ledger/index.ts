...
export function runRogueLedgerGame(terminal: Terminal): RogueLedgerController {
  ...
  function renderWorkbench(lines: string[], current: GameState, theme: string): void {
    const transaction = currentTransaction(current); box(lines, 3, 5, 35, 17, 'ACTIVE BUILD', theme); box(lines, 40, 5, 57, 17, 'INCOMING TRANSACTION', theme);
    ...
    if (transaction) { at(lines, 43, 8, `◆ ${transaction.title.slice(0, 48)}`); at(lines, 43, 10, transaction.description.slice(0, 48)); at(lines, 43, 12, `SOURCE: ${transaction.source.slice(0, 36)}`); at(lines, 14, 14, `${transaction.baseCredits >= 0 ? '+' : ''}${transaction.baseCredits}  ${transaction.tags.map(tagLabel).join(' · ')}`); transaction.visibleClauses.forEach((clause, i) => at(lines, 43, 14 + i, clause.slice(0, 50))); at(lines, 43, 18, 'B Book   C Capitalize   D Defer   R Reserve   X Decline'); }
    if (current.phase === 'preview' && current.preview) { box(lines, 12, 9, 74, 9, 'ENTRY PROJECTION — ENTER TO COMMIT', '\x1b[1;93m'); at(lines, 16, 12, current.preview.trace.slice(0, 3).join('   ')); at(lines, 16, 14, `FINAL ${current.preview.finalCredits >= 0 ? '+' : ''}${current.preview.finalCredits}   AUDIT ${current.preview.auditDelta >= 0 ? '+' : ''}${current.preview.auditDelta}   STANDING +${current.preview.standingDelta}`); at(lines, 16, 16, current.preview.liabilities.length ? `↳ DUE Q${current.preview.liabilities[0]!.dueQuarter}: -${current.preview.liabilities[0]!.amount}` : 'NO SCHEDULED LIABILITY'); }
  ...
  controller.stop = () => { if (!running) return; running = false; if (renderInterval) clearInterval(renderInterval); keyListener?.dispose(); originalStop(); };
