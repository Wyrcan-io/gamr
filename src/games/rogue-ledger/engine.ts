...
export interface GameState {
  seed: number; mode: 'tutorial' | 'standard'; tutorialStep: number | null; phase: Phase; quarter: number; cash: number; profit: number; target: number; floor: number;
...
export function evaluateEntry(state: GameState, transaction: Transaction, treatment: Treatment): EntryResolution {
  ...
  let treatmentCredits = treatment === 'book' ? transaction.baseCredits : treatment === 'capitalize' ? Math.round(transaction.baseCredits * 0.35) : treatment === 'reserve' ? Math.round(transaction.baseCredits * 0.6) : 0;
  ...
  const scheduled = treatment === 'capitalize' ? -Math.round(Math.abs(transaction.baseCredits) * 0.7) : treatment === 'defer' ? -Math.round(Math.abs(transaction.baseCredits) * 1.1) : treatment === 'reserve' ? Math.round(Math.abs(transaction.baseCredits) * 0.4) : 0;
  if (scheduled) trace.push(`SCHEDULE ${scheduled > 0 ? 'RECEIVABLE' : 'LIABILITY'} ${scheduled > 0 ? '+' : ''}${scheduled} DUE Q${state.quarter + 1}`);
  const liabilities = scheduled ? [{ id: `${transaction.id}-schedule`, amount: scheduled, dueQuarter: state.quarter + 1, label: transaction.title }] : [];
...
export type Command = { type: 'dismissBriefing' } | { type: 'selectTreatment'; treatment: Treatment } | { type: 'confirmEntry' } | { type: 'cancelPreview' } | { type: 'dismissResult' } | { type: 'chooseOffer'; offerId: string } | { type: 'continueReport' } | { type: 'restartQuarter' } | { type: 'restartRun'; seed?: number };
...
function closeQuarter(state: GameState): void {
  ...
  for (const liability of due) { state.cash += liability.amount; state.profit += liability.amount; state.notice = `${liability.amount >= 0 ? 'RECEIVABLE COLLECTED' : 'LIABILITY PAID'}: ${liability.label} ${liability.amount >= 0 ? '+' : ''}${liability.amount}`; }
  ...
  if (state.quarter >= (state.mode === 'tutorial' ? 1 : 4)) { state.phase = 'ending'; state.notice = state.mode === 'tutorial' ? 'INDUCTION COMPLETE. THE ROWS RECONCILE.' : 'ANNUAL AUDIT COMPLETE. THE BOOKS SURVIVED.'; return; }
...
export function applyCommand(state: GameState, command: Command): GameState {
  switch (command.type) {
    ...
    case 'cancelPreview': if (state.phase === 'preview') { state.preview = null; state.selectedTreatment = null; state.phase = 'working'; } break;
    case 'confirmEntry': if (state.phase === 'preview' && state.preview) { const result = state.preview; state.cash += result.finalCredits; state.profit += result.finalCredits; state.audit = Math.max(0, Math.min(12, state.audit + result.auditDelta)); state.standing = Math.max(0, Math.min(100, state.standing + result.standingDelta)); state.liabilities.push(...result.liabilities); state.history.push({ ...result, quarter: state.quarter }); state.lastResult = result; if (state.tutorialStep !== null) state.tutorialStep = Math.min(4, state.tutorialStep + 1); state.notice = result.finalCredits >= 0 ? `BOOKED ${result.finalCredits >= 0 ? '+' : ''}${result.finalCredits} CREDITS.` : `BOOKED ${result.finalCredits} CREDITS.`; state.phase = 'result'; } break;
